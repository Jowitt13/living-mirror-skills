...
微信导出的语音是 `.silk`（SILK 编码）。每个 package 都附带 `batch_transcribe_wechat_silk.py` 这个微信专用脚本：它用 `pilk` 把 SILK 直接解码成 16kHz，在 CPU 多核上并行预解码全部文件，再跑批量 FunASR 推理，把 GPU 利用率从约 9% 拉到接近 100%。它会从文件名恢复 `timestamp` / `sender_wxid`；传 `--me-wxid` 可自动推断 `is_self`。
...
Python 3.13 安装注意：先装其他依赖，再 `pip install funasr --no-deps`，因为 `editdistance` 没有预编译包会导致安装整笔回滚。详见 `docs/local-voice-ingestion.md`。
...
2. 蒸馏框架层
   Part A：自我记忆 11 维度
   Part B：人格五层
   证据分级 + CONFLICT 追踪 + Correction + 版本管理 + v0.6 动态镜像规则
...
Living Mirror v0.6 把动态判断层统一成一个正式版本：不只问「这个人是什么样」，还要问「这个判断在什么情境下成立」。重要洞察需要写清楚：
...
v0.6 还加入了机器可读 schema、自动质检脚本、合成 demo 产物、可视化模板和仓库更新日志。
...
可以这样检查 Markdown 自画像或结构化 JSON 洞察：
...
8. 应用 v0.6 动态镜像规则：人类理解维度、状态/特质、情境权重、三段置信度、事实/解释/命名、反证索引、用户语言优先、可推翻条件。
9. 运行 `quality_check_distillation.py` 做结构质检。
10. 每个主题报告完成后让用户复核。
11. 合并成 `self-portrait-YYYY-MM.md`。
12. 更新 `manifest.json`、`changelog.md`、`conflicts.md`、`corrections.md`。
