...
> Living Mirror v0.9 是统一版自我蒸馏核心框架。它可以从杂乱记录开始，也可以从零数据访谈开始，把碎片整理成自画像、复核问题、行动实验、关系地图和可公开分享的脱敏模板。
...
Living Mirror v0.9 把此前的入门、隐私、复核、行动、关系、导出和社区模板能力正式并入统一核心框架。从 v0.9 开始，新的 manifest、模板、自画像和框架文档都应使用 `framework_version: "v0.9"`。

| 版本 | 重点 | 解决什么问题 |
|---|---|---|
| v0.4 | 纵向主题扩展 | 从 10 主题扩展到 15 主题。 |
| v0.5 | 敏感主题可选 | 增加第 16 主题：亲密 / 性表达，默认关闭，必须明确同意。 |
| v0.6 | 动态镜像规则 | 增加情境权重、三段置信度、事实/解释/命名分离、用户语言优先、反证索引。 |
| v0.7 | 入门与信任 | 冷启动访谈、数据体检、同意范围、隐私等级、脱敏、遗忘 / 删除规则。 |
...
这个模块不内置语音模型，也不写死某个模型名称。请在运行时通过 `--model` 参数或 `LIVING_MIRROR_ASR_MODEL` 环境变量指定你自己的本地语音识别模型。
...
三套版本共享同一套 v0.9 框架、references、scripts 和 templates。
